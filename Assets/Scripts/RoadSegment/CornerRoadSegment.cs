...
public class CornerRoadSegment : BaseRoadSegment
{
    ...
    public CornerRoadSegment(RoadPoint pointA, float roadWidth, float pitch, float roll, float radius, float angle) : base(pointA, roadWidth)
    {
        ...
        Vector3 bForward = Vector3.Cross(upwards, centerToB);
        ...
        Debug.DrawLine(pointA.position, circleCenter, Color.red, 20);
        Debug.DrawLine(pointB.position, circleCenter, Color.red, 20);
    ...
    public override MeshData GenerateMesh(int subdivision, int baseIndex)
    {
        ...
        for (int i = 0; i <= subdivision; ++i)
        {
            ...
            Vector3 pointForward = Vector3.Cross(upwards, position - circleCenter);
            Quaternion pointRotation = Quaternion.LookRotation(pointForward, upwards);

            Vector3 left = position + pointRotation * Vector3.left;
            Vector3 right = position + pointRotation * Vector3.right;
